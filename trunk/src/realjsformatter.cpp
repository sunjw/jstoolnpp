...
void RealJSFormatter::Go()
{
	...
	while(GetToken())
	{
		...
		{
		case REGULAR_TYPE:
			PutToken(m_tokenA.code); // 正则表达式直接输出，前后没有任何样式
			break;
		case COMMENT_TYPE_1:
		case COMMENT_TYPE_2:
			if(m_tokenA.code[1] == '*')
...
void RealJSFormatter::ProcessOper(bool bHaveNewLine, char tokenAFirst, char tokenBFirst)
{
		...
		m_tokenA.code == ".")
	{
		...
		GetStackTop(m_blockStack, topStack);
		if(m_tokenA.code == ")" && !m_brcNeedStack.back() &&
			...
			topStack == JS_SWITCH || topStack == JS_CATCH))
		{
			...
			if(StackTopEq(m_blockStack, JS_WHILE)) //m_blockStack.top() == JS_WHILE
			{
				...
				{
					m_blockStack.push_back(JS_WHILE);
					++m_nIndents;
				}
			}
			else
		...
		if(m_tokenA.code == "(" || m_tokenA.code == "[")
		{
			...
			if(topStack == JS_ASSIGN)
			{
				if(!m_bAssign)
					...
					m_blockStack.push_back(JS_HELPER);
			}
			m_blockStack.push_back(m_blockMap[m_tokenA.code]);
			++m_nIndents;
		}
	...
	if(m_tokenA.code == "{")
	{
			...
			topStack == JS_ASSIGN)
		{
			...
			{
				m_blockStack.push_back(JS_HELPER); // 压入一个 JS_HELPER 统一状态
			}
		}
		...
		if(bPrevFunc)
		{
			m_blockStack.pop_back(); // 弹掉 JS_FUNCTION
		...
		if(bPrevFunc)
		{
			m_blockStack.push_back(JS_FUNCTION); // 压回 JS_FUNCTION
		}

		m_blockStack.push_back(m_blockMap[m_tokenA.code]); // 入栈，增加缩进
		++m_nIndents;
		...
		if(m_tokenB.code == "}")
		{
			...
			{
				PutToken(m_tokenA.code, string(" ")); // 这些情况下，前面补一个空格
			}
			else
			{
				PutToken(m_tokenA.code);
	...
	if(m_tokenA.code == "}")
	{
		...
		if((!bHaveNewLine && m_tokenB.code != ";" && m_tokenB.code != ",")
			...
			PutToken(m_tokenA.code, leftStyle, strRight); // 为 else 准备的空格
		else
	...
	PutToken(m_tokenA.code, string(" "), string(" ")); // 剩余的操作符都是 空格oper空格
}
...
void RealJSFormatter::ProcessString(bool bHaveNewLine, char tokenAFirst, char tokenBFirst)
{
	...
	if(m_tokenA.code == "function")
	{
		...
		m_blockStack.push_back(m_blockMap[m_tokenA.code]); // 把 function 也压入栈，遇到 } 弹掉
	}
	...
	{
		// 等待 ()，() 到来后才能加缩进
		m_brcNeedStack.push_back(false);
		m_blockStack.push_back(m_blockMap[m_tokenA.code]);
