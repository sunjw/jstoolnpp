...
FBL.ns(function () {
	with (FBL) {
		...
		Firebug.Debugger = extend(Firebug.ActivableModule, {
			...
			{
				var state = {
					breakOnErrors: Firebug.breakOnErrors
				};
			...
			stop: function (context, frame, type, rv) {
				...
				try {
					...
					var depth = fbs.enterNestedEventLoop({
						onNest: bindFixed(this.startDebugging, this, context)
					});
					// For some reason we don't always end up here
					if (FBTrace.DBG_UI_LOOP)
						FBTrace.sysout("debugger.stop, depth:" + depth + " context:" + context.getName());
				} catch (exc) {
			...
			clearAllBreakpoints: function (context) {
				if (context) {
					...
					fbs.enumerateBreakpoints(null, {
						call: function (url, lineNo, bp) // null means all urls
						{
			...
			startDebugging: function (context) {
				if (FBTrace.DBG_UI_LOOP)
					FBTrace.sysout("startDebugging enter context.stopped:" + context.stopped + " for context: " + context.getName() + "\n");
				try {
				...
				if (FBTrace.DBG_UI_LOOP)
					FBTrace.sysout("startDebugging exit context.stopped:" + context.stopped + " for context: " + context.getName() + "\n");
			},
			...
			showPanel: function (browser, panel) {
				if (panel && panel.name == "script") // this test on name is a sign that this code belongs in panel.show()
				{
					...
					if (FBTrace.DBG_PANELS)
						FBTrace.sysout("debugger.showPanel this.location:" + this.location);
				}
			},
			...
			onThrow: function (frame, rv) {
				...
				try {
					...
					if (!isCatch) {
						...
						if (FBTrace.DBG_BP)
							FBTrace.sysout("debugger.onThrow reset context.thrownStackTrace", context.thrownStackTrace.frames);
					} else {
						if (FBTrace.DBG_BP)
					...
					if (trace.length > 1) // top of stack is [0]
					{
						...
						for (var i = 1; i < trace.length; i++) {
							...
							if (FBTrace.DBG_ERRORS && FBTrace.DBG_STACK)
								FBTrace.sysout("debugger.isCatchFromPreviousThrow " + curFrameSig + "==" + preFrameSig + "\n");
			...
			onError: function (frame, error) {
				...
				try {
					if (FBTrace.DBG_ERRORS)
						FBTrace.sysout("debugger.onError: " + error.errorMessage + " in " + (context ? context.getName() : "no context"), error);
			...
			onTopLevelScriptCreated: function (frame, outerScript, innerScripts) {
				if (FBTrace.DBG_TOPLEVEL)
					FBTrace.sysout("debugger(" + this.debuggerName + ").onTopLevelScriptCreated script.fileName=" + outerScript.fileName + "\n");
				var context = this.breakContext;
				delete this.breakContext;
				...
				if (FBTrace.DBG_TOPLEVEL)
					FBTrace.sysout("debugger.onTopLevelScriptCreated frame.script.tag=" + frame.script.tag + " has url=" + url);
				...
				if (isInline) // never true see above
				{
					...
					{
						if (FBTrace.DBG_SOURCEFILES)
							FBTrace.sysout("debugger.onTopLevelScriptCreated reuse sourcefile=" + sourceFile.toString() + " -> " + context.getName() + " (" + context.uid + ")" + "\n");
						...
						this.watchSourceFile(context, sourceFile);
						if (FBTrace.DBG_SOURCEFILES)
							FBTrace.sysout("debugger.onTopLevelScriptCreated create sourcefile=" + sourceFile.toString() + " -> " + context.getName() + " (" + context.uid + ")" + "\n");
					}
				}
			...
			onToggleBreakpoint: function (url, lineNo, isSet, props) {
				...
				{
					if (FBTrace.DBG_BP)
						FBTrace.sysout("debugger(" + this.debuggerName + ").onToggleBreakpoint ignoring toggle for " + props.debuggerName + " target " + lineNo + "@" + url + "\n");
					return;
				}
			...
			checkDynamicURLhasBP: function (context) {
				...
				for (var url in context.sourceFileMap) {
					...
					if (sourceFile.isEval() || sourceFile.isEvent()) {
						fbs.enumerateBreakpoints(url, {
							call: function setDynamicIfSet(url, lineNo) {
			...
			createSourceFileForFunctionConstructor: function (caller_frame, ctor_script, context) {
				...
				if (FBTrace.DBG_SOURCEFILES)
					FBTrace.sysout("debugger.onNewFunction sourcefile=" + sourceFile.toString() + " -> " + context.getName() + "\n");
			...
			getURLFromLastLine: function (context, lines) {
				...
				if (m) {
					...
					if (m[1] && m[1].indexOf('://') == -1) {
						...
						if (m[1].charAt(0) != '/')
							m[1] = '/' + m[1]; // prepend leading slash if necessary
			...
			loadedContext: function (context) {
				...
				var watchPanelState = Firebug.getPanelState({
					name: "watches",
					context: context
				});
