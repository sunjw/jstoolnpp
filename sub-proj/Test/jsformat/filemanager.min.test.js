var FileManager = {
	...
	viewItemCheck: function () {
		...
		FileManager.setButton("toolbarDelete", "images/toolbar-delete-disable.gif", FileManager.doNothing, "disable", "");
		...
		if (checkedItemsCount > 0) {
			FileManager.setButton("toolbarCut", "images/toolbar-cut.gif", FileManager.clickCut, "", "disable");
			FileManager.setButton("toolbarCopy", "images/toolbar-copy.gif", FileManager.clickCopy, "", "disable");
			FileManager.setButton("toolbarDelete", "images/toolbar-delete.gif", FileManager.clickDelete, "", "disable");
			if (checkedItemsCount == 1) {
				FileManager.setButton("toolbarRename", "images/toolbar-rename.gif", FileManager.clickRename, "", "disable");
			}
		}
	},
	...
	sendAjaxOper: function (oper) {
		...
		$.post("func/post.func.php", {
			...
			if (data == "ok" && FileManager.isSearch == false) {
				...
				FileManager.setButton("toolbarPaste", "images/toolbar-paste-disable.gif", FileManager.doNothing, "disable", "");
			}
		});
	...
	changeMainViewListHeight: function () {
		...
		if (FileManager.isIE && $.browser.version < 8) {
			...
			mainViewListHeight = windowHeight - mainViewListOffset.top
				-footerHeight - 30;
			mainViewListHeight = mainViewListHeight > FileManager.miniMainViewHeight ? mainViewListHeight : FileManager.miniMainViewHeight;
	...
	initToolbar: function () {
		...
		if (buttons.filter(".toolbarForward").hasClass("disable")) {
		...
		buttons.filter(".toolbarPaste").hasClass("disable") ? null : buttons.filter(".toolbarPaste").click(FileManager.clickPaste);
		if (FileManager.isSearch) {
			buttons.filter(".toolbarUp").addClass("disable");
			buttons.filter(".toolbarNewFolder").addClass("disable");
			buttons.filter(".toolbarUpload").addClass("disable");
		} else {
			buttons.filter(".toolbarNewFolder").click(FileManager.clickNewFolder);
			buttons.filter(".toolbarUpload").click(FileManager.clickUpload);
		}
		buttons.hover(FileManager.toolbarButtonMouseIn, FileManager.toolbarButtonMouseOut);
		$("#toolbar form#searchForm input[type='submit']").hover(FileManager.toolbarButtonMouseIn, FileManager.toolbarButtonMouseOut);
